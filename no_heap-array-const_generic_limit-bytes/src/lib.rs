...
/// nucleotides. Also, we want to compare [`Rna`] types regardless of `M`.
#[derive(Clone)]
pub struct Rna<const M: usize = DEFAULT_MAX_NUCLEOTIDES>([u8; M]);
...
impl<const M: usize> Rna<M> {
    ...
    pub fn new_from_iter(mut rna_iter: impl Iterator<Item = char>) -> Result<Self, usize> {
        ...
        //@TODO change: loop until end of input
        let result = Self(core::array::from_fn(|_| {
            rna_iter.next().expect("nucleotide") as u8
        }));
        assert!(rna_iter.next().is_none(), "Not enough space, or too long RNA source.");
