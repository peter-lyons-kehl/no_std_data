...
#[test]
fn test_various_limits_work_together() {
    assert_eq!(
        <dna::DnaImpl<3>>::new("CGA").unwrap(),
        <dna::DnaImpl<8>>::new("CGA").unwrap()
    );
    assert_eq!(
        <dna::DnaImpl<3>>::new("CGA").unwrap().into_rna(),
        <dna::DnaImpl<8>>::new("CGA").unwrap().into_rna()
    );
}
